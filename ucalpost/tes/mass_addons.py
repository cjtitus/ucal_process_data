...
def _compute_peak_samplenumber(self):
    # Monkeypatch for mass out of date with latest sp...
    if self.data is None:
        self.read_segment(0)
    peak_idx = (
        self.data[:, self.cut_pre : self.nSamples - self.cut_post].argmax(axis=1)
        + self.cut_pre
    )
    self.peak_samplenumber = int(sp.stats.mode(peak_idx)[0])
    self.p_peak_index.attrs["peak_samplenumber"] = self.peak_samplenumber
    return self.peak_samplenumber


mass.core.channel.MicrocalDataSet._compute_peak_samplenumber = (
    _compute_peak_samplenumber
)
...
def data_loadStateLabels(self):
    ...
    startTimes, stateLabels = np.loadtxt(
        experimentStateFilename, skiprows=1, delimiter=", ", unpack=True, dtype=str
    )
...
def data_shortname(self):
    ...
    return (
        mass.ljh_util.ljh_basename_channum(
            os.path.split(self.datasets[0].filename)[-1]
        )[0]
        + ", %g chans" % ngoodchan
    )
...
def data_hists(
    self, bin_edges, attr="p_energy", t0=0, tlast=1e20, category={}, g_func=None
):
    ...
    bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])
    countsdict = {
        ds.channum: ds.hist(bin_edges, attr, t0, tlast, category, g_func)[1]
        for ds in self
    }
...
def plot_hist(
    ...
    if axis is None:
        ...
        axis = plt.gca()
    x, y = self.hist(
        bin_edges, attr, category=category, g_func=g_func, stateMask=stateMask
    )
...
def annotate_lines(axis, label_lines, label_lines_color2=[], color1="k", color2="r"):
    ...
    for i, label_line in enumerate(label_lines):
        ...
        if yscale == "linear":
            axis.annotate(
                label_line,
                (energy, (1 + i) * plt.ylim()[1] / float(1.5 * n)),
                xycoords="data",
                color=color1,
            )
        elif yscale == "log":
            axis.annotate(
                label_line,
                (energy, np.exp((1 + i) * np.log(plt.ylim()[1]) / float(1.5 * n))),
                xycoords="data",
                color=color1,
            )
    for j, label_line in enumerate(label_lines_color2):
        ...
        if yscale == "linear":
            axis.annotate(
                label_line,
                (energy, (2 + i + j) * plt.ylim()[1] / float(1.5 * n)),
                xycoords="data",
                color=color2,
            )
        elif yscale == "log":
            axis.annotate(
                label_line,
                (energy, np.exp((2 + i + j) * np.log(plt.ylim()[1]) / float(1.5 * n))),
                xycoords="data",
                color=color2,
            )
...
def ds_linefit(
    ...
    if bin_edges is None:
        bin_edges = np.arange(
            nominal_peak_energy - dlo, nominal_peak_energy + dhi, binsize
        )
    ...
    bin_centers, counts = self.hist(
        bin_edges, attr, t0, tlast, category, g_func, stateMask=stateMask
    )
...
def samepeaks(bin_centers, countsdict, npeaks, refchannel, gaussian_fwhm):
    ...
    peak_locations, peak_intensities = mass.find_local_maxima(
        refcounts, peak_intensities
    )
...
def ds_cut_calculated(ds):
    ...
    ds.cuts.cut_parameter(
        ds.p_pretrig_rms,
        ds.hdf5_group["calculated_cuts"]["pretrig_rms"][:],
        "pretrigger_rms",
    )
    ds.cuts.cut_parameter(
        ds.p_postpeak_deriv,
        ds.hdf5_group["calculated_cuts"]["postpeak_deriv"][:],
        "postpeak_deriv",
    )
...
def ds_plot_ptmean_vs_time(ds, t0, tlast):
    plt.figure()
    plt.plot(
        ds.p_timestamp[ds.good()] - ds.p_timestamp[0], ds.p_pretrig_mean[ds.good()]
    )
...
class PredictedVsAchieved:
    ...
    def achieved(self):
        return np.array(
            [
                self.fitters[ds.channum].last_fit_params_dict["resolution"][0]
                for ds in self.data
            ]
        )
    ...
    def plot(self):
        ...
        plt.title(
            "median predicted %0.2f, median achieved %0.2f\n%s"
            % (med_pred, med_ach, self.data.shortname())
        )
...
class TestPlotAndHistMethods(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        self.data = mass.TESGroupHDF5(
            "/Users/oneilg/Documents/molecular movies/Horton Wiring/horton_2017_07/20171006_B.ljh_pope.hdf5"
        )
    ...
    def test_data_hists(self):
        ...
        self.assertTrue(
            all(countsdict[self.ds.channum] == self.ds.hist(self.bin_edges)[1])
        )
    ...
    def test_linefit_pass_fitter(self):
        fitter = self.ds.linefit(
            mass.MnKAlphaFitter(), bin_edges=np.arange(5850, 5950), attr="p_energy"
        )
        self.assertTrue(fitter.success)
...
def plot_hist2d(self, attr1, attr2, bins1, bins2, norm=mpl.colors.LogNorm(), **kws):
    ...
    counts, x_edges, y_edges, _ = plt.hist2d(
        a1, a2, bins=[bins1, bins2], norm=norm, **kws
    )
    plt.xlabel(attr1)
    plt.ylabel(attr2)
    plt.colorbar()
...
def plot_pulses_by_energy(self, e_centers_ev, indss, xlim=None):
    ...
    for i, (e_ev, inds) in enumerate(zip(e_centers_ev, indss)):
        ...
        if e_ev == 700:
            lw = 2
        plt.plot(
            np.arange(self.nSamples) * self.timebase,
            self.read_trace(inds[0]),
            label=f"{e_ev:0.1f} eV",
            lw=lw,
        )
    plt.xlabel("time (s)")
    ...
    plt.legend()
    plt.title(
        f"{self.shortname()}, predicted={self.predicted_fwhm_at_target_energy:.2} eV"
    )
    if xlim is not None:
        plt.xlim(xlim)
...
def plot_slew_rate(self, e_centers_ev, indss, i_hi, i_lo, n_trace_avg):
    ...
    plt.plot(
        e_ev_matching_delta_mix,
        np.array(delta_mix) * slew_per_delta_mix,
        "o",
        label="up",
    )
    plt.plot(
        e_ev_matching_delta_mix,
        -np.array(delta_mix_min) * slew_per_delta_mix,
        "o",
        label="-down",
    )
